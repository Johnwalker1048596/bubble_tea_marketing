...
BLACK_LIST = [
    ...
    '草莓', '蔓越莓', '蘋果', '芒果', '葡萄柚', '檸檬', '百香果', '金桔' # 防呆：單純寫水果名的不抓
]
...
# ==========================================
def run_deep_miner():
    print("⛏️ 啟動 ELT 第三層：裝甲升級版探勘 + ID 完美對齊...")
    menu_dict = load_menu_dict("beverage_with_fruit_column.xlsx")
    
    with engine.connect() as conn:
        # 🚨 關鍵修復：絕對不再動 %OCR%，只審判空值跟 Deep-Mined 的垃圾！
        df = pd.read_sql(text("""
        ...
        with engine.begin() as tx:
            ...
            """))
            print(f"🧹 平台安全去重完成！清理了 {result.rowcount} 筆重複資料。")
            
            # 🚀 終極強迫症福音：重新對齊 ID (1, 2, 3...)
            tx.execute(text("ALTER TABLE marketing_content DROP COLUMN id;"))
            tx.execute(text("ALTER TABLE marketing_content ADD COLUMN id SERIAL PRIMARY KEY;"))
            print(f"📏 ID 重新對齊完畢！現在你的資料庫像新的一樣漂亮，完全連續無跳號！")
