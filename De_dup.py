...
BLACK_LIST = [
    ...
    '濃郁', '奶香', '得獎', '茶湯', '清香', '沁涼', '鮮果', '喝茶', '回家', '路上', '無論', '每一', '不只', '一杯', '每天'
]
...
# ==========================================
def run_deep_miner():
    print("⛏️ 啟動 ELT 第三層：裝甲升級版探勘 + 終極去重 (De-dup)...")
    ...
    with engine.connect() as conn:
        ...
        with engine.begin() as tx:
            ...
            print(f"✨ 探勘寫入完成，準備執行資料庫去重...")

            # 2. 🛡️ 終極去重陣法 (De-dup)
            # 找出同一個文案 (final_text)，並且飲料名稱一樣的資料，砍掉多餘的！
            # REGEXP_REPLACE 用來拔掉 [Text] 或 [Deep-Mined] 標籤，這樣 [Text]玫瑰水 跟 [Deep-Mined]玫瑰水 就會被視為重複並刪除。
            ...
            print(f"🧹 終極去重完成！成功清理了 {result.rowcount} 筆重複的飲料資料。")
